...
var WebSocketClient = require('./websocket').WebsocketClient;
var Log = require('log');
...
exports.loadTest = function(options, callback)
{
	...
	else if (options.url.startsWith('ws:') || options.url.startsWith('wss:'))
	{
		constructor = WebSocketClient;
	}
	else
	{
		log.error('Invalid URL %s, must be http://, https://, wss:// or ws://', options.url);
		return;
	}
...
var Operation = function(options, Constructor, callback)
{
	...
	self.callback = function(error, result, next)
	{
        ...
        if (options.statusCallback) {
            options.statusCallback(self.latency.getResults(), result);
        }
	};
